...
코딩 스탠다드는 코드 몽키 레벨에서는 왜 이렇게 작성했는지에 대한 설명을 할 필요도 없고 그냥 코딩 스탠다으에 주어진 내용 대로 코드를 짜면 된다고 합니다.

하지만 아래 45가지의 코딩 스탠다다에는 분명히 그렇게 해야 되는 이유가 존재 합니다.
...
## 7. namespaces는 파스칼 케이스[^1]를 사용한다.
